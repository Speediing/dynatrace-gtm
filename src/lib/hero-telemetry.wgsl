...
fn agentComputer(p: vec2f) -> f32 {
  let q = rotate(p, -0.04);
  let monitor = sdRoundedBox(q, vec2f(0.20, 0.14), 0.026);
  let screen = sdRoundedBox(q, vec2f(0.165, 0.105), 0.012);
  var mark = opSmoothSub(monitor, screen, 0.006);
  let neck = sdBox(q - vec2f(0.0, 0.17), vec2f(0.022, 0.035));
  let base = sdRoundedBox(q - vec2f(0.0, 0.21), vec2f(0.09, 0.012), 0.006);
  mark = opSmoothUnion(mark, neck, 0.008);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let green = vec3f(0.278431, 0.682353, 0.560784);
  let blue = vec3f(0.321569, 0.552941, 0.933333);
  let purple = vec3f(0.509804, 0.380392, 0.811765);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let brand = mix(mix(green, blue, uv.x), purple, uv.y * 0.6);
